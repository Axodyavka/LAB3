...
function applyTheme(theme) {
    console.log(`РџСЂРёРјРµРЅРµРЅРёРµ С‚РµРјС‹: ${theme}`);
...
function applyFontSize(size) {
    console.log(`РџСЂРёРјРµРЅРµРЅРёРµ СЂР°Р·РјРµСЂР° С€СЂРёС„С‚Р°: ${size}`);
    document.documentElement.setAttribute('data-font-size', size);
    localStorage.setItem('accessibilityFontSize', size);
}

function applyAccessibilitySettings() {
    console.log('РџСЂРёРјРµРЅРµРЅРёРµ РЅР°СЃС‚СЂРѕРµРє РґРѕСЃС‚СѓРїРЅРѕСЃС‚Рё');
...
export function applyLanguage(lang, caller = 'unknown') {
    if (isApplyingLanguage) {
        console.log(`applyLanguage РїСЂРѕРїСѓС‰РµРЅ (СѓР¶Рµ РІС‹РїРѕР»РЅСЏРµС‚СЃСЏ) РѕС‚ ${caller}, lang: ${lang}`);
        return;
    }
    isApplyingLanguage = true;
    console.log(`applyLanguage РІС‹Р·РІР°РЅ РѕС‚ ${caller}, lang: ${lang}, DOM РіРѕС‚РѕРІ: ${document.readyState}`);
    try {
        if (!translations) {
            console.error('РћР±СЉРµРєС‚ РїРµСЂРµРІРѕРґРѕРІ РЅРµ Р·Р°РіСЂСѓР¶РµРЅ, РїРѕРІС‚РѕСЂ С‡РµСЂРµР· 100РјСЃ');
            setTimeout(() => {
        ...
        if (!translations[lang]) {
            console.warn(`РЇР·С‹Рє ${lang} РЅРµ РїРѕРґРґРµСЂР¶РёРІР°РµС‚СЃСЏ, РѕС‚РєР°С‚ РЅР° 'en'`);
            lang = 'en';
        ...
        elements.forEach(el => {
            ...
            if (translation) {
                ...
                console.warn('РџРµСЂРµРІРѕРґ РЅРµ РЅР°Р№РґРµРЅ РґР»СЏ:', el.getAttribute('data-i18n'), 'РІ СЌР»РµРјРµРЅС‚Рµ:', el);
            }
        });
...
function showAccessibilityPanel() {
    console.log('РџРѕРєР°Р· РїР°РЅРµР»Рё РґРѕСЃС‚СѓРїРЅРѕСЃС‚Рё');
    ...
    panel.querySelectorAll('.font-size-button').forEach(btn => {
        ...
        btn.addEventListener('click', () => {
            ...
            applyFontSize(tempFontSize); // РњРіРЅРѕРІРµРЅРЅРѕРµ РїСЂРёРјРµРЅРµРЅРёРµ
...
export function applyVisibility() {
    console.log('РџСЂРёРјРµРЅРµРЅРёРµ РІРёРґРёРјРѕСЃС‚Рё');
...
function initializeVisibilityCheckbox(attempt = 0, maxAttempts = 10) {
    console.log(`РРЅРёС†РёР°Р»РёР·Р°С†РёСЏ visibilityCheckbox, РїРѕРїС‹С‚РєР° ${attempt + 1}`);
    const visibilityCheckbox = document.getElementById('visibility');
    if (visibilityCheckbox) {
        console.log('visibilityCheckbox РЅР°Р№РґРµРЅ:', visibilityCheckbox);
        ...
        newVisibilityCheckbox.addEventListener('change', function() {
            ...
            if (this.checked && !isHomePage) {
                console.log('Р РµР¶РёРј РґРѕСЃС‚СѓРїРЅРѕСЃС‚Рё РґРѕСЃС‚СѓРїРµРЅ С‚РѕР»СЊРєРѕ РЅР° РіР»Р°РІРЅРѕР№ СЃС‚СЂР°РЅРёС†Рµ');
                this.checked = false;
                return;
            }
            const visibility = this.checked ? 'accessibility' : 'visible';
            console.log(`РџРµСЂРµРєР»СЋС‡РµРЅРёРµ РІРёРґРёРјРѕСЃС‚Рё РЅР°: ${visibility}`);
        ...
        console.warn(`visibilityCheckbox РЅРµ РЅР°Р№РґРµРЅ, РїРѕРІС‚РѕСЂ (${attempt + 1}/${maxAttempts})`);
        setTimeout(() => initializeVisibilityCheckbox(attempt + 1, maxAttempts), 500);
    } else {
        console.error('visibilityCheckbox РЅРµ РЅР°Р№РґРµРЅ РїРѕСЃР»Рµ РјР°РєСЃРёРјСѓРјР° РїРѕРїС‹С‚РѕРє');
    }
}

function initializeSettingsToggle() {
    console.log('РРЅРёС†РёР°Р»РёР·Р°С†РёСЏ РїРµСЂРµРєР»СЋС‡Р°С‚РµР»СЏ РЅР°СЃС‚СЂРѕРµРє');
    const settingsToggle = document.querySelector('.settings-toggle');
    if (settingsToggle) {
    ...
    links.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            if (isHomePage && href && !href.includes('index.html') && !href.includes('#')) {
                console.log('РЎР±СЂРѕСЃ СЂРµР¶РёРјР° РґРѕСЃС‚СѓРїРЅРѕСЃС‚Рё РґР»СЏ РЅР°РІРёРіР°С†РёРё');
                localStorage.setItem('visibility', 'visible');
                applyVisibility();
            }
        });
...
document.addEventListener('headerLoaded', () => {
    console.log('Р—Р°РіРѕР»РѕРІРѕРє Р·Р°РіСЂСѓР¶РµРЅ');
